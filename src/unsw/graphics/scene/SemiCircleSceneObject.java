...
public class SemiCircleSceneObject extends PolygonalSceneObject {

	private float myRadius;
	private Point2D myCenter;

	//Create a SemiCircleSceneObject with centre (0,0) and radius 1
	public SemiCircleSceneObject(SceneObject parent, Color fillColor, Color lineColor) {
		// call constructor of parent class, input the given values to constructor, and create a "null" polygon
		super(parent, new Polygon2D(new ArrayList<Point2D>(16)), fillColor, lineColor);
		
		myCenter = new Point2D(0,0); 	// set centre to (0,0)
		myRadius = 1;					// set radius to 1 
	}

	//Create a SemiCircleSceneObject with centre (0,0) and a given radius
	public SemiCircleSceneObject(SceneObject parent, float radius, Color fillColor, Color lineColor) {
		// call constructor of parent class, input the given values to constructor, and create a "null" polygon
		super(parent, new Polygon2D(new ArrayList<Point2D>(16)), fillColor, lineColor);
	...
	public List<Point2D> setPolygonPoints(Point2D center, float radius) {
        ...
        for (int i = 0; i < 16; i++) {
        	...
        	x = center.getX() + radius * Math.cos(theta) * -1; 
        	y = center.getY() + radius * Math.sin(theta) * -1; 
	...
	public void setSemiCircle(List<Point2D> points) {
    ...
    public void drawSelf(GL3 gl, CoordFrame2D frame) {
		...
		// set semi circle points in polygon with center and radius 
        setSemiCircle(setPolygonPoints(myCenter, myRadius));
